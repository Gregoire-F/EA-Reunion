...
function afficherEntreprises(entreprises) {
  ...
  entreprises.forEach(entreprise => {
    ...
    card.className = "max-w-sm p-3 bg-white border border-gray-200 rounded-lg shadow-sm text-[clamp(0.75rem,3vw,1rem)]";

    card.innerHTML = `
      <img class="w-full h-auto object-cover rounded mb-2" src="${acf.o3.url}" alt="${acf.entreprises}"><img>
